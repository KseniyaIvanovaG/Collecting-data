...
db = client['vacancies']
vacancies_catalog = db.vacancies_catalog

for vacancy in vacancies_list:
    if vacancies_catalog.find_one({'link': vacancy['link']}):   # проверяем есть ли уже вакансия с такой ссылкой
        print(f"Document with link = {vacancy['link']} already exists")
    else:
        vacancies_catalog.insert_one(vacancy)   # если нет, то добавляем

for vacancies_list in vacancies_catalog.find({}):
    pprint(vacancies_list)


# Поиск и вывод на экран вакансии с заработной платой больше введённой суммы

required_salary = int(input("Введите заработную плату"))
for vacancies_list in vacancies_catalog.find({'$or': [{'salary_min': {'$gt': required_salary}},
                                                      {'salary_max': {'$gt': required_salary}}]}):
    pprint(vacancies_list)
