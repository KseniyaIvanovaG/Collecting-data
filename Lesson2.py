# https://hh.ru/search/vacancy?search_field=name&search_field=company_name&search_field=description&text=python&clusters=true&ored_clusters=true&enable_snippets=true
...
pages = dom.find('a',{'class': 'bloko-button'})['href']
for page in pages:
    url2 = base_url + pages
    ...
    for vacancy in vacancies:
        ...
        if not vacancy_salary:
            ...
            if vacancy_salary[0] == 'до':
                salary_min = None
                salary_max = str(vacancy_salary[1]) + str(vacancy_salary[2])
                salary_currency = vacancy_salary[-1]
            elif vacancy_salary[0] == 'от':
                salary_min = str(vacancy_salary[1]) + str(vacancy_salary[2])
                salary_max = None
                salary_currency = vacancy_salary[-1]
            else:
                salary_min = str(vacancy_salary[0]) + str(vacancy_salary[1])
                salary_max = str(vacancy_salary[3]) + str(vacancy_salary[4])
                salary_currency = vacancy_salary[-1]
